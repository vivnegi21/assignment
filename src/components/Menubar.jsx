    ...
    <div className='py-4 p-3 flex mx-auto flex-col gap-4 w-full min-h-screen'>
      ...
      <div className='flex-col gap-4'>
        <Link to='/dashboard/admin' onClick={() => setLinkBar('admin')}>
          <p className={`p-3 w-full text-white rounded-lg text-lg text-center flex gap-2 ${(linkbar === 'admin') ? 'bg-[#FF6666]' : ''}`}>
            <UserRound /> Admin Management
          </p>
        </Link>
        <Link to='/dashboard/roles' onClick={() => setLinkBar('roles')}>
          <p className={`p-3 w-full text-white rounded-lg text-lg text-center flex gap-2 ${(linkbar === 'roles') ? 'bg-[#FF6666]' : ''}`}>
            <UserCog2 /> Role Management
          </p>
        </Link>
        <Link to='/' onClick={() => sessionStorage.clear()}>
          <p className={`p-3 w-full text-white rounded-lg text-lg text-center flex gap-2`}>
            <UserCog2 /> Logout
          </p>
        </Link>
      </div>
